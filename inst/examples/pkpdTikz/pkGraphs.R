...
texCode <- list( g1=list(
              ...
              pkgs=paste(build_usepackage(pkg = list('tikz'),uselibrary = useLib),collapse='\n')),
              g2=list(
                bodyTikz=paste(readLines(system.file('examples/pkpdTikz/pdModel.tex',package = 'texPreview')),collapse='\n'),
                pkgs=paste(build_usepackage(pkg = list('tikz'),uselibrary = useLib1),collapse='\n')),
              g3=list(
                bodyTikz=paste(readLines(system.file('examples/pkpdTikz/pkModel.tex',package = 'texPreview')),collapse='\n'),
                pkgs=paste(build_usepackage(pkg = list('tikz'),uselibrary = useLib1),collapse='\n'))
            )

tex_preview(obj = texCode$g1$bodyTikz,usrPackages = texCode$g1$pkgs)
tex_preview(obj = texCode$g2$bodyTikz,usrPackages = texCode$g2$pkgs)
tex_preview(obj = texCode$g3$bodyTikz,usrPackages = texCode$g3$pkgs)